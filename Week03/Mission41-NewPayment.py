...
def cal_new_payment(age, amount):
    ...
    if age >= 60:  #Check if user is eligible for discount
        new_amount = (95/100) * amount#Calculate new payment amount, round up to integer
        print(amount) #Modify to display original payment amount
        print(new_amount) #Modify to display new payment amount
        return new_amount #Do not remove this line that returns the new amount
    else:
        print("Not entitled to discount") #Modify to display message indicating not entitled to discount
...
print() #Modify to display the amount obtained from function call
